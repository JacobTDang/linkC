...
- Seeded mapping, verbatim: claude `light: haiku`, `standard: sonnet`, `deep: opus`; codex `light: gpt-5.6-luna`, `standard: gpt-5.6-sol`, `deep: gpt-6-astra`; agy `light: gemini-3.8-flash-low`, `standard: gemini-3.8-flash-medium`, `deep: gemini-3.1-pro-high`. Default tier for every agent: `standard`. Cursor has no mapping. An empty entry means that tier is unconfigured and delegation refuses loudly. Every codex id was confirmed by a live `codex exec --model <id>` run: `gpt-6-sol` and `gpt-6-luna` were rejected with HTTP 400, and `gpt-5.6-sol` / `gpt-5.6-luna` answered.
