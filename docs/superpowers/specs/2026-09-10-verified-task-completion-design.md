...
8. **A signal death refuses the gate (§5.2, §8.3).** `ProcessResult` reports the signal that stopped a process. When a signal kills the test command — a crash, an out-of-memory kill, or an outside `kill` — the gate refuses with `gate failed: command was killed (signal N)`, and verification fails with `tests failed at <s7> (signal N)`. A login shell can run its last command in place, so a signal death can reach the runner directly instead of as exit status 128 + N.
