...
- **Changed on disk.** Before writing, linkC checks the file is still what it read. If something else changed it — a `git pull`, a hand edit — the write is refused, the Board says the map changed on disk, and offers to reload. linkC never overwrites a change it has not seen. **Superseded by `2026-09-23-agent-board-tools-design.md`:** such a save now merges instead of refusing; the refuse-and-lock behaviour described here remains only as the last-resort fallback for a file that keeps changing underneath a save that just merged.
