...
public enum BoardDrill {
    ...
    public static func detail(of part: String, onBoard parent: String?, workspacePath: String) throws -> String {
        ...
        let takenSlugs = Set(fileNames.compactMap(BoardSlug.slug(fromFileName:)))
    ...
    public static func open(_ slug: String, workspacePath: String) throws -> BoardMap {
        ...
        let catalog = try BoardCatalog.load(workspacePath: workspacePath, projectName: "")
        guard let entry = catalog.entry(for: slug), entry.linked else {
            return loaded.map
        }
